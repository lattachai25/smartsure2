...
(function() {
    var a = CKEDITOR.htmlParser.fragment.prototype,
        b = CKEDITOR.htmlParser.element.prototype;
    ...
    function c(u) {
        var v = u.children,
            ...
            A,
            B = /list-style-type:(.*?)(?:;|$)/,
            C = CKEDITOR.plugins.pastefromword.filters.stylesFilter;
        x = u.attributes;
    ...
    var d = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i,
        e = /^(?:\b0[^\s]*\s*){1,4}$/,
        f = "^m{0,4}(cm|cd|d?c{0,3})(xc|xl|l?x{0,3})(ix|iv|v?i{0,3})$",
        g = new RegExp(f),
        h = new RegExp(f.toUpperCase()),
        i = {
            decimal: /\d+/,
            "lower-roman": g,
            "upper-roman": h,
    ...
    function n(u) {
        ...
        for (var x = 0; x < v; ++x)
            for (
                var y = l[x], z = y[1].length; u.substr(0, z) == y[1]; u = u.substr(z)
            )
                w += y[0];
        return w;
    }
    ...
    var p = 0,
        q = null,
        r,
        s = (CKEDITOR.plugins.pastefromword = {
            utils: {
                ...
                resolveList: function(u) {
                    var v = u.attributes,
                        w;
                    if (
                        (w = u.removeAnyChildWithName("cke:listbullet")) &&
                        w.length &&
                        (w = w[0])
                    ) {
                        u.name = "cke:li";
                ...
                getStyleComponents: (function() {
                    var u = CKEDITOR.dom.element.createFromHtml(
            ...
            filters: {
                ...
                elementMigrateFilter: function(u, v) {
                    return function(w) {
                        ...
                        CKEDITOR.tools.extend(
                            w.attributes,
                            CKEDITOR.tools.clone(x.attributes)
                        );
                ...
                styleMigrateFilter: function(u, v) {
                    ...
                    return function(x, y) {
                        var z = new CKEDITOR.htmlParser.element(null),
                            A = {};
                        A[v] = x;
                        w(u, A)(z);
            ...
            getRules: function(u) {
                var v = CKEDITOR.dtd,
                    w = CKEDITOR.tools.extend({}, v.$block, v.$listItem, v.$tableContent),
                    x = u.config,
                    y = this.filters,
                    z = y.falsyFilter,
                    A = y.stylesFilter,
                    B = y.elementMigrateFilter,
                    C = CKEDITOR.tools.bind(
                        this.filters.styleMigrateFilter,
                        this.filters
                    ),
                    D = this.utils.createListBulletMarker,
                    E = y.flattenList,
                    F = y.assembleList,
                    G = this.utils.isListBulletIndicator,
                    H = this.utils.isContainingOnlySpaces,
                    I = this.utils.resolveList,
                    J = function(O) {
                        O = CKEDITOR.tools.convertToPx(O);
                        return isNaN(O) ? O : O + "px";
                    },
                    K = this.utils.getStyleComponents,
                    L = this.utils.listDtdParents,
                    M = x.pasteFromWordRemoveFontStyles !== false,
                ...
                return {
                    ...
                    elements: {
                            ...
                            if (CKEDITOR.env.gecko) {
                                ...
                                if (Q) {
                                    ...
                                    y.applyStyleFilter = function(S) {
                                        var T = R["*"] ? "*" : S.name,
                                            U = S.attributes && S.attributes["class"],
                                            V;
                                        if (T in R) {
                                            V = R[T];
                        ...
                        p: function(O) {
                            ...
                            if (x.enterMode == CKEDITOR.ENTER_BR) {
                                ...
                                B(
                                    x["format_" + (x.enterMode == CKEDITOR.ENTER_P ? "p" : "div")]
                                )(O);
                        },
                        div: function(O) {
                            var P = O.onlyChild();
                        ...
                        font: function(O) {
                            ...
                            if ("font" == R.name) {
                                ...
                                Q = Q || "";
                                if (P.color) {
                                    P.color != "#04162b " && (Q += "color:" + P.color + ";");
                                    delete P.color;
                                }
                                if (P.face) {
                                ...
                                if (P.size) {
                                    Q +=
                                        "font-size:" +
                                        (P.size > 3 ? "large" : P.size < 3 ? "small" : "medium") +
                                        ";";
                                    delete P.size;
                                }
                                O.name = "span";
                        ...
                        span: function(O) {
                            ...
                            if (G(O)) {
                                ...
                                if (R) {
                                    var S = D(R, Q),
                                        T = O.getAncestor("span");
                                    if (
                                        T &&
                                        / mso-hide:\s*all|display:\s*none /.test(T.attributes.style)
                                    )
                            ...
                            if (W)
                                ...
                                A([
                                    ...
                                    [
                                        /^font-family$/,
                                        null, !M ? C(x.font_style, "family") : null,
                                    ],
                                    [
                                        /^font-size$/,
                                        null, !M ? C(x.fontSize_style, "size") : null,
                                    ],
                                    [
                                        /^color$/,
                                        null, !M ? C(x.colorButton_foreStyle, "color") : null,
                                    ],
                                    [
                                        /^background-color$/,
                                        null, !M ? C(x.colorButton_backStyle, "color") : null,
                                    ],
                                ])(W, O) || "";
                            return null;
                        },
                    ...
                    comment: !CKEDITOR.env.ie ?
                        ...
                        function(O, P) {
                            var Q = O.match(/<img.*?>/),
                                R = O.match(/^\[if !supportLists\]([\s\S]*?)\[endif\]$/);
                            if (R) {
                                var S = R[1] || (Q && "l."),
                                    T = S && S.match(/>(?:[(]?)([^\s]+?)([.)]?)</);
                                return D(T, S);
                            ...
                            if (CKEDITOR.env.gecko && Q) {
                                var U = CKEDITOR.htmlParser.fragment.fromHtml(Q[0])
                                    .children[0],
                                    V = P.previous,
                                    W =
                                    V &&
                                    V.value.match(/<v:imagedata[^>]*o:href=['"](.*?)['"]/),
                                    X = W && W[1];
    ...
    t.prototype = {
        toHtml: function(u) {
            var v = CKEDITOR.htmlParser.fragment.fromHtml(u, false),
                w = new CKEDITOR.htmlParser.basicWriter();
